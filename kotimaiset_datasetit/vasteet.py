...
def cached_execute_query(query, params_tuple=None):
    ...
    if params_tuple:
        ...
        params = None
    df, error = execute_query(query, params)
    if error:
...
def get_filter_options():
    ...
    df_event, error = execute_query(event_query)
    if error:
        print(f"Database error: {error}")
        return [], []
    df_hake, error = execute_query(hake_query)
    if error:
        print(f"Database error: {error}")
        return [], []
...
def load_data(selected_event_type="All", selected_hake="All"):
    ...
    df, error = execute_query(query, params=params)
    if error:
        print(f"Database error: {error}")
        return pd.DataFrame()
    return df
...
def get_data_ranges():
    ...
    try:
        ...
        if not isinstance(date_range_df, pd.DataFrame) or date_range_df.empty or pd.isna(date_range_df['min_date'].iloc[0]):
            return None, None, [], [], []
