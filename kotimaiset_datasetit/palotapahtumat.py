...
def palotapahtumat_etl(event_type_filter=None):
    ...
    try:
        summary, error = execute_query(query)
        if error:
...
def create_fire_analysis_chart(axis_choice, selected_event_types):
    ...
    try:
        ...
        all_event_types_df, error = execute_query(event_types_query)
        if error:
            print(f"Database error: {error}")
            return None, f"Error creating chart: {str(error)}", []
        
        if isinstance(all_event_types_df, pd.DataFrame) and not all_event_types_df.empty:
