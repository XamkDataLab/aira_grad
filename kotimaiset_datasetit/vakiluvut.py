...
def load_municipality_data():
    ...
    population_df, error = execute_query(population_query)
    if error:
        print(f"Database error: {error}")
        return pd.DataFrame(), []
    incidents_df, error = execute_query(incidents_query)
    if error:
        print(f"Database error: {error}")
        return pd.DataFrame(), []
    
    if not isinstance(population_df, pd.DataFrame) or not isinstance(incidents_df, pd.DataFrame):
        return pd.DataFrame(), []
