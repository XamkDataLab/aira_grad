import gradio as gr
from russia import *
from kotimaiset_datasetit.kotimaiset_datasetit_tabs import build_kotimaiset_datasetit_tab
...
with gr.Blocks(title="Concept Display App", theme=gr.themes.Soft(), css="""
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap');
    
    * {
        font-family: 'Inter', sans-serif !important;
    }
    /* Make main (top-level) tabs stand out */
    .main-tabs button {
    ...
    /* Subtabs should be slightly smaller / less prominent */
...
""") as demo:
    ...
    with gr.Row(equal_height=True, elem_id="header-row"):
        ...
        with gr.Column(scale=8):
            gr.Markdown("")  # Spacer to push logos to the left
    
    # Tabs with Home tab containing the icon selection
    # Top-level tabs (give them a CSS class so we can style them separately)
    with gr.Tabs(elem_classes="main-tabs"):
        with gr.Tab("Koti"):
            
            # Two options side by side with large icons
            with gr.Row():
                # Left option - Bank/Government
...
Vastaa käyttäjän kysymykseen yllä olevien dokumenttien ja keskusteluhistorian perusteella."""
        ...
        with gr.Tab("Ulkomaiset aineistot"):
            # Nested tabs inside (mark as subtabs)
